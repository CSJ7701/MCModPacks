...
function isPositionLoaded(level, x, y, z) {
    let block = level.getBlock(x, y, z)
    return block && block.id !== 'minecraft:air' && block.id !== 'minecraft:void_air'
}
...
function parseBookConfig(pages) {
    ...
    // Helper to extract plain text lines from raw page string
    function getPageLines(pageJson) {
	...
	return rawText.replace(/§./g, '').split('\n').map(l => l.trim()).filter(l => l.length > 0)
    }

    // Validate requirement 1: First non-empty line across all pages must start with "particle:"
    let firstLine = null
    for (let page of pages) {
	let lines = getPageLines(page)
	if (lines.length > 0) {
	    firstLine = lines[0]
	    break
	}
    ...
    if (!firstLine || !firstLine.toLowerCase().startsWith('particle:')) {
	return null // Reject books without "particle: " prefix
    }
	...
	let lines = getPageLines(pageJson)
	lines.forEach(line => {
...
function updateLecternState(level, blockPos, server) {
	...
	if (SavedLecternKeys.delete(key)) persistLecternKeys(server)
	return
    }
	...
	LecternEmitters.delete(key)
	if (SavedLecternKeys.delete(key)) persisteLecternKeys(server)
	return
    }
    ...
    if (!config) {
	LecternEmitters.delete(key)
	if (SavedLecternKeys.delete(key)) peristLecternKeys(server)
	return
    }
...
LevelEvents.loaded(event => {
	...
	keys.forEach(k => SavedLecternKeys.add(k))
    } catch (e) {
...
ServerEvents.tick(event => {
	...
	SavedLecternKeys.forEach(key => {
	    ...
	    if (isPositionLoaded(level, x, y, z)) {
		updateLecternState(level, { x: x, y: y, z: z }, server)
	    }
	})
    }
